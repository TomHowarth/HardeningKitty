...
def apply_value(finding, config_methods):
    ...
    try:
        if method == 'sysctl':
            ...
            return config_methods.set_selinux_mode(recommended_value)
            ...
            if recommended_value.lower() in ['present', 'enabled']:
                ...
                if '/' in argument:  # port/protocol format
                    return config_methods.add_firewalld_rule(zone, port=argument)
                else:
                    return config_methods.add_firewalld_rule(zone, service=argument)
            else:
                if '/' in argument:
                    return config_methods.remove_firewalld_rule(zone, port=argument)
                else:
                    return config_methods.remove_firewalld_rule(zone, service=argument)
