...
class ConfigMethods:
    ...
    def get_package_status(package_name):
        ...
        output, error = utils.run_command(f"rpm -q {package_name}", check=False)

        if output and not output.startswith('package') and not error:
            return "installed"
    ...
    def install_package(package_name):
        ...
        output, error = utils.run_command(f"dnf install -y {package_name}", check=False)
        return error is None

    @staticmethod
    def remove_package(package_name):
        """Remove package using dnf/yum"""
        output, error = utils.run_command(f"dnf remove -y {package_name}", check=False)
        return error is None
    ...
    def get_selinux_status():
        """Get SELinux status"""
        output, error = utils.run_command("getenforce", check=False)
        if output:
            return output.lower()  # enforcing, permissive, disabled
        return "unknown"
    ...
    def set_selinux_mode(mode):
        ...
        if mode.lower() != 'disabled':
            output, error = utils.run_command(f"setenforce {mode}", check=False)
            if error:
                return False

        # Update config file for persistence
        config_file = "/etc/selinux/config"
        return ConfigMethods.set_config_file_value(config_file, "SELINUX", mode.lower())
    ...
    @staticmethod
    def get_firewalld_rule(zone, service_or_port):
        """Check if firewalld rule exists"""
        output, error = utils.run_command(f"firewall-cmd --zone={zone} --list-all", check=False)
        if not output:
            return None
    ...
    @staticmethod
    def add_firewalld_rule(zone, service=None, port=None):
        """Add firewalld rule"""
        if service:
            cmd = f"firewall-cmd --zone={zone} --add-service={service} --permanent"
        elif port:
            cmd = f"firewall-cmd --zone={zone} --add-port={port} --permanent"
        else:
            return False
        ...
        # Reload firewall
        utils.run_command("firewall-cmd --reload", check=False)
        return True

    @staticmethod
    def remove_firewalld_rule(zone, service=None, port=None):
        """Remove firewalld rule"""
        if service:
            cmd = f"firewall-cmd --zone={zone} --remove-service={service} --permanent"
        elif port:
            cmd = f"firewall-cmd --zone={zone} --remove-port={port} --permanent"
        else:
            return False
