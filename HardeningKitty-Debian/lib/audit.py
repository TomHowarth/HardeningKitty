...
def get_current_value(finding, config_methods):
    ...
    try:
        if method == 'sysctl':
            ...
            return config_methods.get_apparmor_status()

        elif method == 'selinux':
            # Fall back to checking if AppArmor is being used instead
            return config_methods.get_apparmor_status()
            ...
            has_rule = config_methods.get_ufw_rule(argument)
            return "present" if has_rule else "absent"

        elif method == 'ufw_status':
            return config_methods.get_ufw_status()
